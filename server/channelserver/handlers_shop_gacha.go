...
func handleMsgMhfEnumerateShop(s *Session, p mhfpacket.MHFPacket) {
	...
	if pkt.ShopType == 2{
	    ...
	    if err != nil {
			...
			stringBytes := append([]byte(stringsupport.MustConvertUTF8ToShiftJIS(gachaName)), 0x00)
			resp.WriteUint8(byte(len(stringBytes)))
			resp.WriteBytes(stringBytes)
			stringBytes = append([]byte(stringsupport.MustConvertUTF8ToShiftJIS(gachaLink0)), 0x00)
			resp.WriteUint8(byte(len(stringBytes)))
			resp.WriteBytes(stringBytes)
			stringBytes = append([]byte(stringsupport.MustConvertUTF8ToShiftJIS(gachaLink1)), 0x00)
			resp.WriteUint8(byte(len(stringBytes)))
			resp.WriteBytes(stringBytes)
			stringBytes = append([]byte(stringsupport.MustConvertUTF8ToShiftJIS(gachaLink2)), 0x00)
...
func handleMsgMhfGetStepupStatus(s *Session, p mhfpacket.MHFPacket) {
	...
	if t.After(midday){	midday = midday.Add(24 * time.Hour) }
